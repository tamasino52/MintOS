...
jmp 0x1000:START    ; CS 세그먼트 레지스터에 0x07C0을 복사하면서, START 레이블로 이동

    strweek:    db 'Sat$Sun$Mon$Tue$Wed$Thu$Fri'
    DA: db '0000/00/00', 0
    week : dw 00
...
START:
    ...
    mov ax, 0x1000  ; 부트 로더의 시작 어드레스(0x7C00)를 세그먼트 레지스터 값으로 변환
...
RESETDISK:                          ; 디스크를 리셋하는 코드의 시작
    ...
    mov si, 0x1020                  ; OS 이미지를 복사할 어드레스(0x10000)를 
                                    ; 세그먼트 레지스터 값으로 변환
