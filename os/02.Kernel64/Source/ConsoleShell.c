...
static void kCreateFileInRootDirectory( const char* pcParameterBuffer )
{
	...
	FILE* pstFile;
    	...
    	fclose( pstFile );
/*	offset=kFindDirectoryEntry(vcFileName,&stEntry);
	kReadRTCTime(&(stEntry.hour),&(stEntry.min),&(stEntry.sec));
	kReadRTCDate(&(stEntry.year),&(stEntry.month),&(stEntry.day),&(stEntry.week));
	kPrintf("%d %d %d",stEntry.hour,stEntry.min,stEntry.sec);
	kPrintf("%d %d %d\n",stEntry.year,stEntry.month,stEntry.day);
	BYTE t=20;
	kMemCpy(&stEntry.hour,&t,sizeof(t));
	kPrintf("%d",stEntry.hour);*/
	...
	int tmpoffset;
		...
		if( pstEntry -> bType == FILESYSTEM_TYPE_DIRECTORY)
		{
			...
			{ 				
        		...
        		kPrintf( "    %s", vcBuffer );
			kPrintf(" %d-%d-%d %d:%d:%d\n",pstEntry->year,pstEntry->month,pstEntry->day,pstEntry->hour,pstEntry->min,pstEntry->sec);
		}
        	else
		{
        		kMemSet( vcBuffer, ' ', sizeof( vcBuffer ) - 1 );
        		...
        		kMemCpy( vcBuffer + 33, vcTempValue, kStrLen( vcTempValue ) + 1 );
      
			kPrintf( "  %s", vcBuffer );
			tmpoffset=kFindDirectoryEntry(pstEntry->d_name,&stEntry);
			kPrintf("  %d-%d-%d %d:%d:%d\n",stEntry.year,stEntry.month,stEntry.day,stEntry.hour,stEntry.min,stEntry.sec);
