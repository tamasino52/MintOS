...
tmp=iLength;
	...
	if( kMemCmp( vcFileName,"a.txt",6 )==0 )
	{
		
		pstFile=fopen(vcFileName,"w");
		kPrintf("%d\n",pstFile->stFileHandle.iDirectoryEntryOffset);
		kGetDirectoryEntryData(0,(pstFile->stFileHandle).iDirectoryEntryOffset,pstEntry);

		
		kPrintf("%s\n",pstEntry->vcFileName);
		kMemCpy(pstEntry->vcFileName,vcdir,iLength);
		kPrintf("%s\n",pstEntry->vcFileName);
		if(kUpdateDirectoryEntry(&(pstFile->stFileHandle))==FALSE)
		{
			kPrintf("%s","uoi");
		}
		fclose(vcFileName);
